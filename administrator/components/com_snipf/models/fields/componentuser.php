...
require_once __DIR__.'/../../helpers/snipf.php';
...
class JFormFieldComponentuser extends JFormFieldList
{
  ...
  public function getOptions()
  {
    ...
    preg_match('#^com_snipf\.([a-zA-Z0-9_-]+)\.filter$#', $this->form->getName(), $matches);
    $itemName = $matches[1];
    ...
    //Person: Some SQL table names are separated with underscore.
    if(array_key_exists($itemName, $this->exceptions)) {
      $itemName = $this->exceptions[$itemName];
    }

    $options = SnipfHelper::getUsers($itemName);
