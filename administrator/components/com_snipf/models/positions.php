...
class SnipfModelPositions extends JModelList
{
  ...
  protected function getListQuery()
  {
    ...
    $query->select($this->getState('list.select', 'p.id, p.name, p.created, p.published,'.
				   'p.created_by, p.checked_out, p.checked_out_time'));

    $query->from('#__snipf_position AS p');
    ...
    //Checks if it's called from a modal window.
    $layout = JFactory::getApplication()->input->get->get('layout', '', 'string');
    if($layout == 'modal') {
      //Displays only the published item in modal windows.
