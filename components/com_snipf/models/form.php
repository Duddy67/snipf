...
require_once JPATH_ADMINISTRATOR.'/components/com_snipf/models/person.php';


//Inherit the backend version.
class SnipfModelForm extends SnipfModelPerson
{
  /**
   ...
   * @var        string
   */
  public $typeAlias = 'com_snipf.person';
  ...
  protected function populateState()
  {
    ...
    $this->setState('person.catid', $app->input->getInt('catid'));
   ...
   *
   * @param   integer  $itemId  The id of the person.
   *
   * @return  mixed  Content item data object on success, false on failure.
   */
  public function getItem($itemId = null)
  {
    $itemId = (int) (!empty($itemId)) ? $itemId : $this->getState('person.id');

    // Get a row instance.
    $table = $this->getTable();

    // Attempt to load the row.
    //Persons: If it's a new item, load function just return true.
    ...
    //Person: params fields are missing on purpose in the xml form as
    //params cannot be set on frontend.
    //All of the person items created on frontend has an empty
    //params value.
    ...
    if($itemId) {
      ...
      $item->persontext = $item->intro_text;
      if(!empty($item->full_text)) {
	$item->persontext .= '<hr id="system-readmore" />'.$item->full_text;
      }
    }
    else { // New item.
      $catId = (int) $this->getState('person.catid');

      if($catId) {
	//Check the change access in this specific category.
	$item->params->set('access-change', $user->authorise('core.edit.state', 'com_snipf.category.'.$catId));
	$item->catid = $catId;
      }
      else { //Check the general change access.
	$item->params->set('access-change', $user->authorise('core.edit.state', 'com_snipf'));
      }
    }
