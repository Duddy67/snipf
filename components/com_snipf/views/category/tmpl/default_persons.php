...
<table class="table category table-striped">
  ...
  <tr>
    <th id="categorylist_header_title">
      <?php echo JText::_('COM_SNIPF_FIELD_LASTNAME_LABEL'); ?>
    </th>
    <?php if($this->params->get('list_show_author')) : ?>
      <th width="25%">
	<?php echo JText::_('COM_SNIPF_HEADING_CREATE_BY'); ?>
      </th>
    <?php endif; ?>
    <?php if($this->params->get('list_show_date')) : ?>
      <th width="15%" id="categorylist_header_date">
	<?php $date = $this->params->get('order_date'); ?>
	<?php echo JText::_('COM_SNIPF_'.strtoupper($date).'_DATE'); ?>
      </th>
    <?php endif; ?>
  </tr>
  </thead>
  ...
  <tbody>
	...
	<?php  //Build the link to the login page for the user to login or register.
		...
		$returnURL = JRoute::_(SnipfHelperRoute::getPersonRoute($item->slug, $item->catid));
		$link = new JUri($link1);
		$link->setVar('return', base64_encode($returnURL));
	      endif; ?>

	<?php if($item->params->get('access-view')) : //Set the link to the person page.
	      $link = JRoute::_(SnipfHelperRoute::getPersonRoute($item->slug, $item->catid));
	  endif; ?>
	  ...
	  <?php if($this->params->get('list_show_date')) : ?>
		...
		<?php echo JText::_('COM_SNIPF_UNMODIFIED'); ?>
	      <?php else : ?>
