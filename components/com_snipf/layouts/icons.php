...
<div class="icons">
  <?php if($canEdit) : ?>
    <div class="btn-group pull-right">
      ...
      <?php // Person the actions class is deprecated. Use dropdown-menu instead. ?>
      <ul class="dropdown-menu">
	<?php if($canEdit) :
	    //First check if the person is checked out by a different user
	    if($item->checked_out > 0 && $item->checked_out != $user->id) :
	      ...
	      $tooltip = JText::_('JLIB_HTML_CHECKED_OUT').' :: '.
			 JText::sprintf('COM_SNIPF_CHECKED_OUT_BY', $checkoutUser->name).' <br /> '.$date;
	  ?>
	   ...
	   <?php else : 
	      ...
	      $url = 'index.php?option=com_snipf&task=person.edit&p_id='.$item->id.'&return='.base64_encode($uri);
	      ?>
	      <li class="edit-icon"><a href="<?php echo JRoute::_($url); ?>"><span class="icon-edit"></span>
				     <?php echo JText::_('COM_SNIPF_EDIT'); ?></a>
	      </li>
	  <?php endif; ?>
	<?php endif; ?>
      </ul>
